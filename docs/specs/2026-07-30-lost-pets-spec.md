...
2. Питомец не найден по коду → `404`. Питомец не в `LOST` → `409 Conflict` («питомец не в поиске»).
...
4. После сохранения — **уведомление владельцу** через существующий `NotificationsService` (и gateway, чтобы прилетело в реальном времени): `type: SIGHTING`, `title: "🐾 {pet.name}: питомца видели!"`, `body` из address/comment, `data: { petId, sightingId, lat, lng }`.
